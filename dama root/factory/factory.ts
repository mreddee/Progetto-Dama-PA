/*
    Design pattern factory, usato per generare gli errori specifici.
    tutte le classi di errori provengono dalla interfaccia Msg,
    che definisce lo status e la stringa della risposta inviata
    dal server al client come risposta alla richiesta
*/ 
...
class ErrPlayerBusy implements Msg {
    getMsg(): { status: number; msg: string; } {
        return {
            status: 402,
            msg: "Error: Opponent is already playing!"
        }
    }
}
...
/** 
  Restituisce una classe di errore specifica in base all'enum dell'input 
**/
let val: Msg;
export function getError(type: ErrorEnum): Msg{
    switch (type)
    {
        ...
        case ErrorEnum.ErrorPlayerTrun:
            ...
            break;
        case ErrorEnum.ErrorPlayerBusy:
            val = new ErrPlayerBusy();
            break;
