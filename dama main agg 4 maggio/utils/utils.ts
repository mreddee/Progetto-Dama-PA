...
* @returns 
*/
export function checkGameOver(pieces1: number, pieces2: number, abbandono1: boolean, abbandono2: boolean) {
    //let isGameClosed: boolean = false;
    if (pieces1==0 || pieces2==0 || abbandono1==true || abbandono2==true) {
        return true;
    }
    else {
        return false;
...
export async function checkIfAdmin(email: string): Promise<boolean> {
    let user: any = await models.Users.findByPk(email, {raw: true});
...
export async function getUserTokens(email: string, res: any): Promise<number> {
    let user: any = await models.Users.findByPk(email, {raw: true});
    return user.token;
}
...
export async function checkIfUsersArePlaying(email1: string, email2: string, res: any): Promise<boolean> {
    let playing1: boolean;
    let playing2: boolean;
    let check: boolean = false;
    try {
        playing1 = await isUserPlaying(email1);
        playing2 = await isUserPlaying(email2);
        check = !playing1 && !playing2;
    }
    catch(e: any) {
        controller.generateControllerErrors(ErrorEnum.InternalServer, e, res);
    }
    return check;
}

export async function decreaseTokens(email: string, payment: number): Promise<void> {
    let user: any = await models.Users.findByPk(email);
